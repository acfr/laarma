...
public:
  ...
  void infer();
  void postprocess(std::vector<Object> &objs, float score_thres = 0.25f,
...
void YOLOv8::make_pipe(bool warmup) {
  ...
  for (auto &bindings : this->input_bindings) {
    void *d_ptr;
    CHECK(
        cudaMallocAsync(&d_ptr, bindings.size * bindings.dsize, this->stream));
...
void YOLOv8::postprocess(std::vector<Object> &objs, float score_thres,
                         float iou_thres, int topk, int num_labels) {
  objs.clear();
...
void YOLOv8::draw_objects(
  ...
  for (auto &obj : objs) {
    cv::Scalar color;
    if (obj.label < COLORS.size())
      color = cv::Scalar(COLORS[obj.label][0], COLORS[obj.label][1],
                         COLORS[obj.label][2]);
    else
    ...
    if (obj.label < CLASS_NAMES.size())
      class_name = CLASS_NAMES[obj.label];
    else
      class_name = "unknown";
    sprintf(text, "%s %.1f%%", class_name.c_str(), obj.prob * 100);
